...
private fun BookDetailContent(
    ...
    // A book not yet released IN FRANCE cannot be marked "read" or "favorite" — only
    // following it (to be notified on release) makes sense at this stage, the same logic as
    // cinema/games. isReleasedInFrance (French edition only, no fallback to the original
    // date) avoids treating a volume already published abroad but not yet translated as
    // "released".
    val isBookReleased = remember(book.id) {
        book.isReleasedInFrance
    }
    ...
    LazyColumn(state = listState, modifier = Modifier.fillMaxSize()) {
        ...
        item {
            ...
            ActionButtons(
                ...
                watchedDisabled = !isBookReleased,
                ...
                favoriteDisabled = !isBookReleased,
