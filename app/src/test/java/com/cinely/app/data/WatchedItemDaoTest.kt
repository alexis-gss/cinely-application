...
class WatchedItemDaoTest {
    ...
    fun observeAll_trie_par_watchedAt_decroissant_le_plus_recent_en_premier() = runBlocking {
        dao.insert(WatchedItem(tmdbId = 1, mediaType = "movie", title = "Ancien", posterPath = null, watchedAt = 1000L))
        dao.insert(WatchedItem(tmdbId = 2, mediaType = "movie", title = "Récent", posterPath = null, watchedAt = 5000L))
        dao.insert(WatchedItem(tmdbId = 3, mediaType = "movie", title = "Milieu", posterPath = null, watchedAt = 3000L))

        val all = dao.observeAll().first()

        assertEquals(listOf("Récent", "Milieu", "Ancien"), all.map { it.title })
    }
