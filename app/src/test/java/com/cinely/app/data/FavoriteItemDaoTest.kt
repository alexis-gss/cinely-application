...
class FavoriteItemDaoTest {
    ...
    fun un_film_et_une_serie_avec_le_meme_tmdbId_sont_bien_distincts_en_favori() = runBlocking {
        ...
        dao.upsert(FavoriteItem(tmdbId = 1, mediaType = "tv", title = "Série homonyme", posterPath = null, addedAt = 2000L))
    ...
    fun observeAll_trie_par_addedAt_decroissant_le_plus_recent_en_premier() = runBlocking {
        dao.upsert(FavoriteItem(tmdbId = 1, mediaType = "movie", title = "Ancien", posterPath = null, addedAt = 1000L))
        dao.upsert(FavoriteItem(tmdbId = 2, mediaType = "movie", title = "Récent", posterPath = null, addedAt = 5000L))

        val all = dao.observeAll().first()

        assertEquals(listOf("Récent", "Ancien"), all.map { it.title })
    }
    ...
    fun retirer_un_favori_movie_ne_supprime_pas_le_favori_tv_homonyme() = runBlocking {
        dao.upsert(FavoriteItem(tmdbId = 1, mediaType = "movie", title = "Film", posterPath = null, addedAt = 1000L))
        dao.upsert(FavoriteItem(tmdbId = 1, mediaType = "tv", title = "Série", posterPath = null, addedAt = 2000L))
